...
## installation
in your hugo project, navigate to the `layouts/shortcodes` folder (create it if it doesn't exist), and paste the following code into `layouts/shortcodes/embed-pdf.html`:
```html
...
Finally if you don't have a `static/js` directory, create one, then run the command in your project root:
```
git submodule add --depth=1 https://github.com/fmgornick/hugo-embed-pdf-shortcode.git static/js/pdf-js
```
